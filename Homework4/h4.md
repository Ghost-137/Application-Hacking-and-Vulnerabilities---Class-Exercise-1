...
# (e) Nora crackme01e
In here I decompiled the crackme01e.64 in ghidra. Then find the main function. Analyze the assembly file and the code. go to the terminal and check with "echo"

![Alt text](<Screenshot from 2026-02-06 18-17-14.png>)

# (f) Nora crackme02

After decompiling with ghidra, I gave the variable a meaningful name. 

![Alt text](image-8.png)

# Analyze the logic

Look at the critical check inside the loop:

if (cVar1 + -1 != (int)*(char *)(user_input + index))

It means the program is not checking if my input equals "password1". It is checking if my input equals "password1" minus 1. It wants the character before the reference character in the ASCII table. 

The cVar2 starts as "p". Then it updates cVar2 + "password1"[index+1]; Basically, it walks through the string "password1" and expects my input to be 1 value lower for every single letter.


So we need to take "password1" and subtract 1 from every character's ASCII value. 

So, the password is "o`rrvnqc0" according to the ascii table.
...
Sources
    ...
    Hammond, J. (2022). Ghidra for Reverse Engineering (PicoCTF 2022 #42 'bbbloat'). [Video].

    Karvinen, T. (2024). Some Disassembly Required. Course Materials.

    Tindall, N. (2023). NoraCodes/crackmes. GitHub repository.

    Eagle, C. & Nance, K. (2020). The Ghidra Book: The Official Ghidra Guide. (Optional Reference).
